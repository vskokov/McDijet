...
//  $Corrresponding Author: vladi@skokov.net
//==============================================================================
#include <cmath>
...
//********Example of Pythia AfterBurner ******
// #include "PythiaAfterBurner.h"  
// use with make pythiaAfterBurner 
//********************************************
#include <iostream>
#include <fstream>
#include <string>
...
const double min_qt_to_Pt_ratio = 1;
...
DIS::DIS(double E_ein, double E_pin, int Ain):E_e(E_ein),E_p(E_pin),A(Ain)
{
    ...
    cout << "# generating interpolating cache..." << endl;
...
//cerr << Q << " "<< W << " " <<  Flux_T(Q2, W2, S)  << " " <<  Flux_L(Q2, W2, S)   << endl ;
    ...
    cout << "# Settings:" << endl;
    cout << "# Num of Events  = " << number_of_events << endl;
    cout << "#        sqrt(s) = " << sqrtS << endl;
    cout << "#              A = " << A << endl;
    cout << "#        sigma_T = " << std::setprecision(5) << std::scientific << IntXS_T << " mb  "<< endl;
    cout << "#        sigma_L = " << std::setprecision(5) << std::scientific << IntXS_L << " mb  "<< endl;
...
int main(int argc, char** argv) {
	...
	AfterBurner * afterBurner = new NoBurner; // Just parton output  
	// AfterBurner * afterBurner = new PythiaAfterBurner; // Pythia After Burner, see include section   
	...
	afterBurner->init(E_e, E_p, A_target);

    for(int i=0; i<number_of_events; i++) {
        vector<double> evt = generator();

        if ((i+1)%1000 == 0 || i==number_of_events-1) {    
            cout << "# Events processed: " <<  i+1 << endl;
        }                                                
        afterBurner->event(evt); // AfterBurner plug-in          
